...
# Extract the animal position relative to the maze quadrants
def get_bp_position_on_region_OF(body_part_matrix, maze_regions_dict, fps=30):
    
    xy = body_part_matrix[:,0:2]  # x,y coords       
    # Create a body part position vector (coded as 0 to N for each one of the quadrants)
    bp_pos_on_region = np.zeros(xy.shape[0],)
    
    # region_value based on the order the user defined each region
    region_value = 0
    # Loop for each region
    for dict_key in maze_regions_dict.keys():
        # Check if the body part is positioned on the specific region
        rectangle = maze_regions_dict[dict_key]      
        # Check whether each point is inside the quadrant currently being analyzed
        results_bool = are_points_inside_polygon(rectangle, xy)        
        # Insert the index regarding the animal's body part on quadrant      
        bp_pos_on_region[results_bool] = region_value # Insert the index regarding the role (+1, since python indexing is quite differente from what I've learned in MATLAB)
        # Add 1 to the region_value so it can represent the next region
        region_value = region_value + 1
        
    return bp_pos_on_region

# Get the time spent on each quadrant and calculate the ratio time on the target quadrant / time on the other quadrants
def get_time_on_target_quadrant(bp_pos_on_quadrant, target=1, fps=30, prob=False):
    ...
    # Just pre-allocate
    ratio_target_minute = np.zeros((2,))
    
    # Also calculate the time by minute of trial  
    if prob is True:
        a = bp_pos_on_quadrant[0:60*fps-1]
        ratio_target_minute[0] = np.count_nonzero(a == target) / (60*fps)
        
        b = bp_pos_on_quadrant[60*fps:120*fps]
        ratio_target_minute[1] = np.count_nonzero(b == target) / (60*fps)
    
    return ratio_target_others, time_on_each_quadrant, ratio_target_minute

# Get the time spent on each quadrant and calculate the ratio time on the target quadrant / time on the other quadrants
def get_time_on_each_maze_region_OF(bp_pos_on_region, region_dict_keys, reference_region='center', fps=30, prob=False):
    # Get a list of each possible region code (0 to N)
    region_codes = np.unique(bp_pos_on_region)
    # Create a zeros array for each region
    time_on_each_region = np.zeros(len(region_codes),)
    # Get the time on each region
    for i in region_codes:
        time_on_each_region[int(i)] = np.count_nonzero(bp_pos_on_region == i)/fps
            
    # Get the reference code
    count = 0
    for k in region_dict_keys:
        if k == reference_region:
            ref_idx = count
            break
        else:
            count = count + 1
            
    # Calculate the ratio target/total
    ratio_reference_others = time_on_each_region[ref_idx]/np.sum(time_on_each_region)
    
    return ratio_reference_others, time_on_each_region
