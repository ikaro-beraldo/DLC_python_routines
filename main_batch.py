...
latency_mode = 'find' # 'escape' or 'find'   
trial_duration=2 # Trial duration in minutes
prob=True # Bool (if it is a prob trial or not)
target=1
target_quadrant=1
exp_min_time = 0.5 # Minimum Time threshold for a hole exploration Seconds
find_time_thresh=1   # Minimum Time exploration threshold for the scape hole to be found
...
for it in range(len(filename)):
    ...
    if latency_mode == 'escape':
        ...
        latency, body_part_matrix_nose, body_part_matrix_body_centre, bp_pos_on_maze, bp_pos_on_maze_filtered\
            = get_trial_latency_to_find_scape(body_part_matrix_nose, body_part_matrix_body_centre, bp_pos_on_maze, bp_pos_on_maze_filtered, time_thresh=find_time_thresh, fps=fps, scape_hole=1)
    ...
    data = pd.DataFrame([[box, ID, group, day, trial, beg, end, latency, p_errors, s_errors, strategy, total_distance, av_speed, ratio_target_others, ratio_target_minute, time_on_each_quadrant]], 
                        columns = ['Box','ID','Group','Day','Trial','Beginning', 'End', 'Latency', 'P_error', 'S_error',
                                       'Strategy', 'Distance', 'Av_speed','Time_on_target','Time_on_target_minute','Time_each_quadrant']) 
...
# STEP 12 --> Save the final datraframe as MATLAB file
import scipy
save_filename = os.path.dirname(filename[it])+'/'+'Final_results'+'.mat'
scipy.io.savemat(save_filename, {'task_parameters':trial_info.to_dict("list")})