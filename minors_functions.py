...
# Function to generate points between the vertices
def generate_points(x, y, x1, y1, n):
    points = []
    for i in range(n + 1):
        xi = x + (x1 - x) * i / n
        yi = y + (y1 - y) * i / n
...
def fit_rectangle_from_multiple_points(points):

    # Create a MultiPoint from the input points
    ...
    rect_vertices = list(min_bounding_rect.exterior.coords)
    
    # Return only the 4 points
    return rect_vertices[0:4]
