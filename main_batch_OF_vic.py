# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 15:46:29 2024
...
maze_info_pixel['box_length'] = (80,60) # (long-side, short-side in cm)
# object_diameter

block_average_sum = 30 # in minutes
...
# speed_info = pd.DataFrame(columns=np.linspace(block_average_sum/60,max_trial_duration/60,int(max_trial_duration/block_average_sum)))
# distance_info = pd.DataFrame(columns=np.linspace(block_average_sum/60,max_trial_duration/60,int(max_trial_duration/block_average_sum)))
maze_regions = pd.DataFrame()
speed_info = pd.DataFrame()
...
for it in range(len(filename)):
    ...
    maze_info_pixel = maze_recreation(maze_info_pixel, position_each_vertex, position_each_PIM)
    # Get the x,y coordinates for each vertex of the grid squares
    grid_square_coords = generate_grid_square_coordinates(position_each_vertex[0:4,:], x_y_grid_size=(5,5))
    ...
    maze_regions_dict, maze_regions_dict_list = model_maze_regions(maze_info_pixel, grid_square_coords, quadrant_info)
    # Model the objects
    ...
    # Fix coordinates inconsistencies based on confidence interval
    teste, exc_frames = fix_frames_confidence(body_centre_coord,conf_threshold)
    ...
    rectangle = position_each_vertex[4:8,:]
    ...
    # plt.plot(body_part_matrix_body_centre[:,0],body_part_matrix_body_centre[:,1])
    # plt.figure
    # plt.plot(np.diff(body_part_matrix_body_centre[:,1]))
    ...
    inst_speed, inst_speed_entire, av_speed = get_inst_speed(body_part_matrix_body_centre, maze_info_pixel, time_win=fps/3, fps=fps)
    ...
    # Average speed and distance sum over 30 minutes blocks
    averaged_speed = average_blocks(inst_speed_entire, fps*60*block_average_sum, alg_type='average',min_block=block_average_sum)
    sum_distance = average_blocks(inst_speed_entire*(1/fps), fps*60*block_average_sum, alg_type='sum',min_block=block_average_sum)
    
    
    # Check whether the animal is moving or not (above 4 cm/s)
    mov = np.where(inst_speed >= 2)[0]
    rest = np.where(inst_speed < 2)[0]
    plt.plot(mov,inst_speed[mov],'.')
    plt.plot(rest,inst_speed[rest],'.')
    
    time_mov = len(mov)/fps
    time_rest = len(rest)/fps
    ...
    video_name = ''.join(basename[0:cut_idx])
    
    # Organize the block averaged/sum series
    
    # Add the videoname
    averaged_speed.insert(0,'video_name',video_name)
    sum_distance.insert(0,'video_name',video_name)
        
    speed_info = pd.concat([speed_info, averaged_speed], ignore_index = True)  
    distance_info = pd.concat([distance_info, sum_distance], ignore_index = True)  
    
    # Maze region information
    # Extract the column names from the dictionary keys
    column_names = list(quadrant_info.keys())
    # Create a DataFrame with the series values as a single row and the dictionary keys as column names
    data = pd.DataFrame([pd.Series(time_on_each_region).values], columns=column_names)
    data.insert(0,'video_name',video_name)
    maze_regions = pd.concat([maze_regions, data], ignore_index = True)

    
    ######### Create a data frame to append to the final dataframe
    data = pd.DataFrame([[video_name, ID, group, day, total_distance, av_speed, ratio_reference_others,time_on_each_region, time_mov, time_rest]], 
                        columns = ['video_name','ID','Group','Day','Distance', 'Av_speed', 'ratio_reference_others','time_on_each_region','time_mov','time_rest']) 
    # makes index continuous
    trial_info = pd.concat([trial_info, data.drop('time_on_each_region', axis=1)], ignore_index = True)  
    ...
    big_plot_OF(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_info_pixel, inst_speed_entire, maze_regions_dict, bp_pos_on_region, save_filename_plot, trial_name=trial_name, trial_data=data, show=True, fps=fps)
...
# Save the final_results as csv 
save_filename = os.path.dirname(filename[it])+'/'+'Final_results'+'.csv'
trial_info.to_csv(save_filename) 

# Save the speed and distance block results as csv 
save_filename = os.path.dirname(filename[it])+'/'+'speed_block'+'.csv'
speed_info.to_csv(save_filename) 
save_filename = os.path.dirname(filename[it])+'/'+'distance_block'+'.csv'
distance_info.to_csv(save_filename) 

# Maze regions results as csv
save_filename = os.path.dirname(filename[it])+'/'+'maze_regions'+'.csv'
maze_regions.to_csv(save_filename) 
