...
def plot_trajectory(axes, body_part_matrix, fps=30):
    # Trajectory itself
    c = colorline(body_part_matrix[:,0], body_part_matrix[:,1], cmap="cool", ax=axes, linewidth=1)
...
def plot_bp_pos_on_region(axes, bp_pos_on_region, maze_regions_dict, trial_data=None, fps=30):
    
    colorline(np.arange(len(bp_pos_on_region))/fps, bp_pos_on_region, cmap="cool", linewidth=1.5, ax=axes)
    axes.set_xlim((0,len(bp_pos_on_region)/fps))
    axes.set_ylim((-0.5,max(bp_pos_on_region)+0.5))
    axes.set_yticks(np.arange(0,max(bp_pos_on_region)+1,1), list(maze_regions_dict))
    axes.set_xlabel("Seconds")
    axes.set_ylabel("Animal Position")
    axes.set_title("Animal position on maze")
    #plt.show()
    
    if not(trial_data is None):
        # Plot a text with the number of errors, and strategy
        axes.text(len(bp_pos_on_region)/fps/50, 3.25, 'Obj 1: '+str(trial_data['time_obj_1'][0])+' Ratio: '+str(trial_data['ratio_1_total'][0]))
        axes.text(len(bp_pos_on_region)/fps/50, 3, 'Obj 2: '+str(trial_data['time_obj_2'][0])+' Ratio: '+str(trial_data['ratio_2_total'][0]))
        axes.text(len(bp_pos_on_region)/fps/50, 2.75, 'Total: '+str(trial_data['time_obj_1'][0]+trial_data['time_obj_2'][0]))
...
def plot_instant_speed_maze_OF(axes, body_part_matrix, inst_speed_entire, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel):
    
    # Plot the maze recreation
    maze_recreation_plot_OF(axes, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Body Centre', show=False, invert=False)
    # Show plots on the selected axes    
    c = colorline(body_part_matrix[:,0], body_part_matrix[:,1], z=inst_speed_entire/inst_speed_entire.max(), cmap="magma", linewidth=1.5, ax=axes)
    cbar = plt.colorbar(c,ax=axes, ticks=[0, 0.5, 1])
    # Get the max, half and min speeds
...
def plot_instant_speed_maze_EPM(axes, body_part_matrix, inst_speed_entire, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel):
    
    # Plot the maze recreation
    maze_recreation_plot_EPM(axes, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Body Centre', show=False, invert=False)
    # Show plots on the selected axes    
    c = colorline(body_part_matrix[:,0], body_part_matrix[:,1], z=inst_speed_entire/inst_speed_entire.max(), cmap="magma", linewidth=1.5, ax=axes)
    cbar = plt.colorbar(c,ax=axes, ticks=[0, 0.5, 1])
    # Get the max, half and min speeds
...
def plot_instant_speed_maze_LD(axes, body_part_matrix, inst_speed_entire, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel):
    
    # Plot the maze recreation
    maze_recreation_plot_LD(axes, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Body Centre', show=False, invert=False)
    # Show plots on the selected axes    
    c = colorline(body_part_matrix[:,0], body_part_matrix[:,1], z=inst_speed_entire/inst_speed_entire.max(), cmap="magma", linewidth=1.5, ax=axes)
    cbar = plt.colorbar(c,ax=axes, ticks=[0, 0.5, 1])
    # Get the max, half and min speeds
...
# Function to call the heatmap and plot it
def plot_heatmap_OF(body_part_matrix, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, ax=None):
    
    plot_range = [[position_each_vertex[4,0] - 100, position_each_vertex[5,0] + 100], [position_each_vertex[4,1] - 100, position_each_vertex[7,1] + 100]]
    ...
    # Plot Maze
    maze_recreation_plot_OF(ax, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Body Centre', show=False, invert=False)
    ax.invert_yaxis()
...
# Function to call the heatmap and plot it
def plot_heatmap_EPM(body_part_matrix, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, ax=None):
    
    # Define the plot range and the heatmap range as well
    min_max_x = [np.min(position_each_vertex[:,0]), np.max(position_each_vertex[:,0])]
    min_max_y = [np.min(position_each_vertex[:,1]), np.max(position_each_vertex[:,1])]
    
    plot_range = [[min_max_x[0] - 100, min_max_x[1] + 100], [min_max_x[0] - 100, min_max_x[1] + 100]]
    ...
    # Plot Maze
    maze_recreation_plot_EPM(ax, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Body Centre', show=False, invert=False)
    ax.invert_yaxis()
...
# Function to call the heatmap and plot it
def plot_heatmap_LD(body_part_matrix, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, ax=None):
    ...
    # Plot Maze
    maze_recreation_plot_LD(ax, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Body Centre', show=False, invert=False)
    ax.invert_yaxis()
...
def plot_time_on_each_region(axes, maze_regions_keys, trial_data):
    # Get the time spent on each maze region
    time_spent = trial_data['time_on_each_region'][0]
    # Plot the spent on each region
    axes.bar(maze_regions_keys, time_spent)

    axes.set_ylabel('Time Spent (sec)')
    axes.set_title('Time spent on each maze region')  
...
def big_plot_OF(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_info_pixel, inst_speed_entire, maze_regions_dict, bp_pos_on_region, save_filename ,trial_name=None, trial_data=None, show=True, fps=30):
    figure, axes = plt.subplots(3,2, figsize=(12.8, 9.6))  # Define the 6 subplots
       
    # 1st PLOT --> the maze and the body part trajectory
    # Recreate the maze
    maze_recreation_plot_OF(axes[0,0], centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Nose', show=False, invert=False)
    # BP trajectory
    plot_trajectory(axes[0,0], body_part_matrix_body_centre, fps=fps)
    axes[0,0].invert_yaxis()
    
    # 2nd PLOT --> the body part position considering holes and regions
    plot_bp_pos_on_region(axes[0,1], bp_pos_on_region, maze_regions_dict, trial_data=None, fps=fps)
    
    # 3rd PLOT --> plot the animal instant speed on maze
    plot_instant_speed_maze_OF(axes[1,0], body_part_matrix_body_centre, inst_speed_entire, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel)
    axes[1,0].invert_yaxis()
    
    # 4th PLOT --> plot the animal instant speed on time
    plot_instant_speed(axes[1,1], inst_speed_entire, body_part_matrix_body_centre, trial_data=None, fps=fps)
    
    # 5th PLOT --> plot the animal spent on a specif part of the maze as a heat map
    plot_heatmap_OF(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, ax=axes[2,0])
    #plot_quadrants_OLR(quadrant_dict, ax=axes[2,0], fps=fps)
    
    # 6th PLOT --> plot the time animal spent on each maze region defined by the user
    plot_time_on_each_region(axes[2,1], maze_regions_dict.keys(), trial_data)
...
def big_plot_EPM(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_info_pixel, inst_speed_entire, maze_regions_dict, bp_pos_on_region, save_filename ,trial_name=None, trial_data=None, show=True, fps=30):
    figure, axes = plt.subplots(3,2, figsize=(12.8, 9.6))  # Define the 6 subplots
       
    # 1st PLOT --> the maze and the body part trajectory
    # Recreate the maze
    maze_recreation_plot_EPM(axes[0,0], centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Nose', show=False, invert=False)
    # BP trajectory
    plot_trajectory(axes[0,0], body_part_matrix_body_centre, fps=fps)
    axes[0,0].invert_yaxis()
    ...
    # 3rd PLOT --> plot the animal instant speed on maze
    plot_instant_speed_maze_EPM(axes[1,0], body_part_matrix_body_centre, inst_speed_entire, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel)
    axes[1,0].invert_yaxis()
    ...
    # 5th PLOT --> plot the animal spent on a specif part of the maze as a heat map
    plot_heatmap_EPM(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, ax=axes[2,0])
    #plot_quadrants_OLR(quadrant_dict, ax=axes[2,0], fps=fps)
...
def big_plot_LD(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_info_pixel, inst_speed_entire, maze_regions_dict, bp_pos_on_region, save_filename ,trial_name=None, trial_data=None, show=True, fps=30):
    figure, axes = plt.subplots(3,2, figsize=(12.8, 9.6))  # Define the 6 subplots
       
    # 1st PLOT --> the maze and the body part trajectory
    # Recreate the maze
    maze_recreation_plot_LD(axes[0,0], centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=False, title='Nose', show=False, invert=False)
    # BP trajectory
    plot_trajectory(axes[0,0], body_part_matrix_body_centre, fps=fps)
    axes[0,0].invert_yaxis()
    ...
    # 3rd PLOT --> plot the animal instant speed on maze
    plot_instant_speed_maze_LD(axes[1,0], body_part_matrix_body_centre, inst_speed_entire, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel)
    axes[1,0].invert_yaxis()
    ...
    # 5th PLOT --> plot the animal spent on a specif part of the maze as a heat map
    plot_heatmap_LD(body_part_matrix_body_centre, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, ax=axes[2,0])
    #plot_quadrants_OLR(quadrant_dict, ax=axes[2,0], fps=fps)
...
# Recreate the Open Field in a previous defined axes
def maze_recreation_plot_OF(axes, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=True, title='Body Centre', show=True, invert=True, video_file=None, video_frame=None):
    ...
    # Plot the maze regions defined by the user
    for key in maze_regions_dict:
        x = maze_regions_dict[key][:,0]
        y = maze_regions_dict[key][:,1]
        axes.fill(x,y,'w',alpha=0.1, edgecolor='k', linewidth=2)
    ...
    ###################################### PERFUMARIAS ###########################################
    # Set the x and y limits
    axes.set_xlim((position_each_vertex[4,0] - 100, position_each_vertex[5,0] + 100))
    axes.set_ylim((position_each_vertex[4,1] - 100, position_each_vertex[7,1] + 100))
...
# Recreate the Elevated Plus Maze in a previous defined axes
def maze_recreation_plot_EPM(axes, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=True, title='Body Centre', show=True, invert=True, video_file=None, video_frame=None):
    ...
    if plot_frame is True:
        ...
        # Redefine the plot limits
        axes.set_xlim((position_each_vertex[5,0] - 100, position_each_vertex[9,0] + 100))
        axes.set_ylim((position_each_vertex[11,1] - 100, position_each_vertex[7,1] - 100))
        
        if invert is True:
            # Invert the yaxis
            axes.invert_yaxis()
    
    # Plot the Maze regions
    # Plot the interior rectangles
    vertex_order = [0,6,7,1,8,9,3,10,11,2,4,5,0]
    axes.plot(position_each_vertex[vertex_order,0],position_each_vertex[vertex_order,1], 'go-', linewidth=2)
    ...
    min_max_x = [np.min(position_each_vertex[:,0]), np.max(position_each_vertex[:,0])]
    min_max_y = [np.min(position_each_vertex[:,1]), np.max(position_each_vertex[:,1])]
    axes.set_xlim((min_max_x[0] - 100, min_max_x[1] + 100))
    axes.set_ylim((min_max_y[0] - 100, min_max_y[1] + 100))
...
# Recreate the Light-Dark Box in a previous defined axes
def maze_recreation_plot_LD(axes, centroid_coords, position_each_vertex, maze_regions_dict, maze_info_pixel, plot_frame=True, title='Body Centre', show=True, invert=True, video_file=None, video_frame=None):
