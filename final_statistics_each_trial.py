...
save_file = "E:\\Barnes Maze - Mestrad\\Resultados DLC - Barnes\\"
...
latency_trial_av = np.zeros((16,len(IDs)))
latency_trial_err = np.zeros((16,len(IDs)))
x = []
count = 0;
...
for ii in np.arange(1,5,1):
    for jj in np.arange(0,len(IDs),1):
        ...
        latency_trial_av[idx,jj] = latency.T
...
# min1_min2_total = trial_info["Time_on_target_minute"].to_numpy()
# for ii in range(len(min1_min2_total)):
#     min1_min2[ii] = min1_min2_total[ii][0] / min1_min2_total[ii][1]
...
savemat("E:\\Barnes Maze - Mestrad\\Resultados DLC - Barnes\\barnes_maze_result_matrix2.mat", mdic)