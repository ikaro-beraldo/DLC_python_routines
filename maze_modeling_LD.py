# -*- coding: utf-8 -*-
"""
Created on Mon Jan  8 16:22:49 2024
...
def get_vertex_positions(dlc_data_frame, bp_names, prev_vertex_position=None):
    ...
    position_each_vertex = np.zeros((6,2))
...
def centroid_inference(vertex_coords):
    ref_vertex = [0,1,4,5]
    x = np.sum(vertex_coords[ref_vertex,0])/len(vertex_coords[ref_vertex,0])
    y = np.sum(vertex_coords[ref_vertex,1])/len(vertex_coords[ref_vertex,1])
...
def maze_recreation(maze_info_pixels, vertex_coords):
    ...
    pixel_dist_vertexs_S = np.empty((3,1))
    
    # Calculate the difference between vertices
    # Short side (the 3 possible combination of vertices)
    for n,i in enumerate(range(0,6,2)):    
    ...
    # Long side (the 2 possible combination of vertices)
    for i in range(2):    
        pixel_dist_vertexs_L[i] = np.sqrt((vertex_coords[i,0]-vertex_coords[i+4,0])**2 + (vertex_coords[i,1]-vertex_coords[i+4,1])**2) # Pixel distance
...
def model_maze_regions(vertex_coords):
    ...
    # Center region
    maze_regions_dict['dark_box'] = vertex_coords[[0,1,3,2],:]
    maze_regions_dict['light_box'] = vertex_coords[[2,3,5,4],:]
